...
(function () {
  ...
  const CHARS = {
    gojo: {
      ...
      skills: {
        ...
        sum: { name: 'Maximum: Purple', ce: 300, cd: 60 },
  ...
  function updateProjectiles(dt) {
    for (let i = projectiles.length - 1; i >= 0; i--) {
      ...
      // Maximum: Purple erases lesser techniques in its path
      if (p.banishes) {
        for (const q of projectiles) {
          if (q === p || q.owner === p.owner || q.life <= 0) continue;
          if (q.pos.distanceTo(p.pos) < p.r + q.r) {
            burst(q.pos.clone(), 0xc06bff, 8, 6, 0.35, 0.6);
            q.life = -1;
            q.groundExplode = false;
            q.onImpact = null;
          }
        }
      }
  ...
  function updateAction(f, dt) {
    ...
    switch (act.name) {
      ...
      case 'maxPurple': {
        ...
        f.aura = 1;
        // blue and red converge into something that should not exist
        if (act.t < 1.6 && Math.random() < 0.85) {
          const hpP = f.pos.clone().addScaledVector(fw, 1.1).setY(1.6);
          const cc = Math.random() < 0.5 ? 0x4fd4ff : 0xff3344;
          spawnP(hpP.clone().add(tmpV.set(rand(-2.5, 2.5), rand(-1.5, 2), rand(-2.5, 2.5))), tmpV.multiplyScalar(-4.5), 0.3, 0.7, cc);
        }
        if (crossed(act, 1.6, dt)) {
          AudioSys.explosion();
          AudioSys.blast(60);
          addShake(0.9, 0.9);
          slowmo.t = 0.4;
          slowmo.factor = 0.35;
          screenFlash('rgba(180,70,255,0.6)', 0.8);
          const start = f.pos.clone().addScaledVector(fw, 2.2).setY(1.6);
          tmpV.subVectors(o.pos, f.pos).setY(0).normalize();
          spawnProjectile({
            owner: f, pos: start, vel: tmpV.clone().multiplyScalar(19),
            r: 4.6, visR: 4.0, dmg: 400, kb: 34, kbUp: 16, knockdown: true, pierce: true,
            color: 0xa22cff, trailColor: 0xc06bff, type: 'orb', life: 4.5, infCost: 120,
            banishes: true,
          });
          flashAt(start, 0xa22cff, 14, 1.0);
          spawnRing(start, 0xa22cff, 9, 0.6, true);
        }
  ...
  /* ================= MAHORAGA ================= */
  // each fighter's trump card lives on the same key
  function trySummon(f) {
    const sk = f.cfg.skills.sum;
    if (!sk || f.cds.sum > 0 || f.ce < sk.ce) return;
    if (f.charKey === 'sukuna') {
      if (summon) return;
      f.ce -= sk.ce;
      f.cds.sum = sk.cd;
      ...
      // Gojo: Maximum Technique — the Shinjuku nuke
      f.ce -= sk.ce;
      f.cds.sum = sk.cd;
      f.state = 'skill';
      f.eyeGlow = 1.8;
      startAction(f, 'maxPurple', 1.9, { armor: true });
      AudioSys.domain();
      AudioSys.charge(1.4);
      announce('MAXIMUM: PURPLE', '“Throughout Heaven and Earth, I Alone Am the Honored One”', 2.4, 'purple');
      addShake(0.3, 1.2);
      screenFlash('rgba(170,60,255,0.35)', 0.8);
    }
  }
  ...
  function updateSummon(dt) {
    ...
    // Maximum: Purple erases even the Divine General
    for (const p of projectiles) {
      if (!p.banishes || p.owner === s.owner) continue;
      const dx = p.pos.x - s.pos.x, dy = p.pos.y - (s.pos.y + 1.8), dz = p.pos.z - s.pos.z;
      if (Math.sqrt(dx * dx + dy * dy + dz * dz) < p.r + 2.4) {
        announce('MAHORAGA ERASED', 'nothing withstands imaginary mass', 1.5, 'purple');
        AudioSys.explosion();
        addShake(0.8, 0.7);
        slowmo.t = 0.5;
        slowmo.factor = 0.3;
        burst(s.pos.clone().setY(2.5), 0xc06bff, 30, 12, 0.8, 1.2);
        spawnRing(s.pos.clone().setY(2), 0xa22cff, 8, 0.6, true);
        despawnSummon(false);
        return;
      }
    }
  ...
  function aiInput(f, dt) {
    ...
    if (ai.t <= 0) {
      ...
      // the trump card
      if (f.cfg.skills.sum && f.cds.sum <= 0 && f.ce >= f.cfg.skills.sum.ce) {
        if (f.charKey === 'sukuna' && !summon &&
            (f.hp < MAX_HP * 0.6 || o.hp < MAX_HP * 0.5) && Math.random() < 0.35) {
          inp.sum = true;
          return inp;
        }
        // Gojo answers Mahoraga with Maximum: Purple — or nukes when desperate
        if (f.charKey === 'gojo' &&
            ((summon && summon.owner !== f && Math.random() < 0.65) ||
             ((f.hp < MAX_HP * 0.5 || o.hp < MAX_HP * 0.4) && Math.random() < 0.3))) {
          inp.sum = true;
          return inp;
        }
      }
